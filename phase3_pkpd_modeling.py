...
class PKPDAnalyzer:
    ...
    def __init__(self, data_path='data/processed/ml_dataset.csv',
        ...
        print(f"  ✓ ML dataset: {self.ml_data.shape}")
        print(f"  ✓ Concentration data: {self.conc_data.shape}")
        print(f"  ✓ Dosing data: {self.dose_data.shape}")
        print(f"  ✓ Patient data: {self.patient_data.shape}")
        print()
    ...
    def calculate_all_pk_indices(self):
        ...
        for idx, row in self.ml_data.iterrows():
            ...
            # Get MIC for this patient
            mic = row['mic_gentamicin']
        ...
        print(f"\n✓ Calculated PK/PD indices for {len(self.pkpd_indices)} patients")
        print(f"\nSummary statistics:")
        ...
        print(f"✓ Saved PK/PD indices to: {output_file}")
        print()
    ...
    def perform_pta_analysis(self, mic_range=None):
        ...
        print(f"✓ Saved PTA results to: {output_file}")
        print()
    ...
    def calculate_cfr(self, mic_distribution=None):
        ...
        print("✓ CFR calculation complete")
        ...
        print(f"✓ Saved CFR results to: {output_file}")
        print()
    ...
    def link_pkpd_to_outcomes(self):
        ...
        # Logistic regression: P(cure) = 1 / (1 + exp(-(β0 + β1*Cmax/MIC)))
        ...
        print(f"  β0 (intercept): {beta0_cure:.3f}")
        print(f"  β1 (Cmax/MIC):  {beta1_cure:.3f}")
        ...
        print(f"  β0 (intercept): {beta0_tox:.3f}")
        print(f"  β1 (trough):    {beta1_tox:.3f}")
        ...
        print(f"\n✓ Saved outcome models to: {output_file}")
        print()
    ...
    def generate_summary_report(self):
        ...
        report.append("Clinical Cure Model: P(cure) = 1/(1 + exp(-(β0 + β1*Cmax/MIC)))")
        report.append(f"  β0 (intercept): {self.outcome_models['cure']['beta0']:.3f}")
        report.append(f"  β1 (Cmax/MIC):  {self.outcome_models['cure']['beta1']:.3f}")
        report.append("")

        report.append("Nephrotoxicity Model: P(AKI) = 1/(1 + exp(-(β0 + β1*Trough)))")
        report.append(f"  β0 (intercept): {self.outcome_models['toxicity']['beta0']:.3f}")
        report.append(f"  β1 (Trough):    {self.outcome_models['toxicity']['beta1']:.3f}")
        report.append("")
    ...
    def run_complete_analysis(self):
        ...
        print("  Run: python3 phase4_machine_learning.py")
        print()
