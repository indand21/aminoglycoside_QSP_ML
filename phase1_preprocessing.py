...
class AminoglycosidePreprocessor:
    ...
    def calculate_pk_pd_metrics(self):
        ...
        # Merge with patient data to get MIC values and drug assignment
        patient_mic = self.data['patient_data'][['patient_id', 'mic_amikacin', 'mic_gentamicin', 'drug']].copy()
        pk_metrics = pk_metrics.merge(patient_mic, on='patient_id', how='left')

        # Drug-specific MIC selection
        pk_metrics['mic_used'] = np.where(
            pk_metrics['drug'] == 'amikacin',
            pk_metrics['mic_amikacin'],
            pk_metrics['mic_gentamicin'])
    ...
    def create_nonmem_dataset(self):
        ...
        patient_cov = self.data['patient_data'][
            ['patient_id', 'age', 'weight', 'height', 'sex',
             'baseline_scr', 'baseline_crcl', 'apache_ii', 'sofa_score', 'drug']
        ].copy()
        ...
        patient_cov['DRUG'] = (patient_cov['DRUG'] == 'amikacin').astype(int)  # 1=amikacin, 0=gentamicin
